...
struct HypothesisReport {
    h5b_0_null: Check,
    h5b_1_safe_escape: Check,
    h5b_2_structured_noise_control: Check,
    h5b_3_holdout_clean_cue: Check,
    h5b_4_reproducibility: Check,
}
...
fn run_experiment() -> Result<ExperimentArtifact, Box<dyn Error>> {
    ...
    let case_ids_ok = cases.iter().all(|case| case.case_id.starts_with("fl5b|"));
    ...
    let validity = ValidityReport {
        ...
        fl5_case_ids_not_reused: Check::from_bool(case_ids_ok),
        protocol_valid: Check::from_bool(protocol_valid),
    };

    let hypotheses = HypothesisReport {
        h5b_0_null: Check::from_bool(none_survived || !escape),
        h5b_1_safe_escape: Check::from_bool(!none_survived && escape),
        h5b_2_structured_noise_control: Check::from_bool(!none_survived && structured),
        h5b_3_holdout_clean_cue: Check::from_bool(!none_survived && clean_safe),
        h5b_4_reproducibility: Check::from_bool(replay_ok),
    };
    ...
    Ok(ExperimentArtifact {
        experiment: "FL-5B",
        protocol: "clean-cue-constrained-basin-escape-v1",
        ...
        fixture: FixtureInfo {
            ...
            description: "FL-5B namespace; 8-node three-pattern Hebbian bank; wrong-basin cues are exact competitors plus single differing-bit flips toward the declared target; clean cues are exact stored patterns; hard clean-cue calibration filter",
        },
        cases,
...
fn build_cases(
    ...
    for target_index in 0..bank.patterns().len() {
        for competitor_index in 0..bank.patterns().len() {
            ...
            for (variant, cue) in cues.into_iter().enumerate() {
                let case_id = format!("fl5b|wb|t{target_index}|c{competitor_index}|k{variant}");
    ...
    for target_index in 0..bank.patterns().len() {
        ...
        let case_id = format!("fl5b|clean|t{target_index}");
        if !seen.insert(case_id.clone()) {
...
fn provenance_fingerprint() -> u64 {
    let manifest = format!(
        "fl5b|nodes={NODE_COUNT}|steps={FIELD_STEPS}|dt={FIELD_DT:.17}|mobility={FIELD_MOBILITY:.17}|tilt={CUE_TILT_RADIANS:.17}|amps={AMPLITUDES:?}|ou={OU_THETAS:?}|seeds={SEEDS:?}|perm={PERM_SEED}|noiselab={NOISELAB_COMMIT}|patterns=3|fixture=fl5b-wb-competitor+diffbit|hard-clean-cue-filter"
    );
...
mod tests {
    ...
    #[test]
    fn fl5b_case_ids_use_dedicated_namespace() {
