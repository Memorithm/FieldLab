...
const REPORT_SCHEMA: &str = "fieldlab.flb2-report.v1";
...
struct HypothesisOutcomes {
    hb2_0_protocol_valid: bool,
    hb2_1_determinate_contrast_exists: bool,
    hb2_2_memory_signatures_pairwise_distinct: bool,
    hb2_3_baselines_support_nonartifact: bool,
    hb2_4_bit_role_separation: bool,
}
...
fn predicate_bank() -> Result<Vec<ComponentThresholdPredicate>, Box<dyn Error>> {
    ...
    if predicates.len() != 16 {
        return Err("FL-B2 predicate count drift".into());
    }
    Ok(predicates)
}
...
fn run_once(source_revision: &str) -> Result<Report, Box<dyn Error>> {
    ...
    if (stiffness - 1.25).abs() > 1.0e-12 {
        return Err(format!("FL-B2 expected stiffness 1.25, got {stiffness}").into());
    }
            ...
            for direction in [-1.0_f64, 1.0_f64] {
                ...
                for step in 0..=STEPS {
                    ...
                    if !error.is_finite() || error > VALIDITY_NORM_SQUARED_TOLERANCE {
                        return Err(format!(
                            "FL-B2 unit-state validity failed for p{memory}/n{probe_node}/{direction_name} at step {step}: {error:e}"
                        )
                        .into());
                    }
                ...
                if states.len() != OBSERVATION_COUNT {
                    return Err("FL-B2 observation count drift".into());
                }
                trajectory_count += 1;
                ...
                for (predicate_index, predicate) in predicates.iter().enumerate() {
                    ...
                    cases.push(CaseRecord {
                        case_id: format!(
                            "flb2|p{memory}|pred{predicate_index}|n{probe_node}|d{direction_name}"
                        ),
                        memory,
                        predicate_index,
                        probe_node,
    ...
    if trajectory_count != 48 {
        return Err("FL-B2 trajectory count drift".into());
    }
    if cases.len() != 768 {
        return Err("FL-B2 case count drift".into());
    }
    ...
    let hb2_1 = determinate_hits > 0;
    let hb2_2 = signatures_pairwise_distinct(&memory_signatures);
    ...
    let hb2_3 = const_false_determinate_hits == 0
        && determinate_hits > const_false_determinate_hits
        && (!perm_distinct || !perm_role_separation);
    let hb2_4 = role_multisets.iter().any(|entry| entry.multisets_differ);
    let hb2_0 = cases.len() == 768
        && trajectory_count == 48
    ...
    Ok(Report {
        ...
        replay_exact: false,
        hypotheses: HypothesisOutcomes {
            hb2_0_protocol_valid: hb2_0,
            hb2_1_determinate_contrast_exists: hb2_1,
            hb2_2_memory_signatures_pairwise_distinct: hb2_2,
            hb2_3_baselines_support_nonartifact: hb2_3,
            hb2_4_bit_role_separation: hb2_4,
        },
    })
}
...
fn run_replayed(source_revision: &str) -> Result<Report, Box<dyn Error>> {
    ...
    if first != second {
        return Err("FL-B2 semantic replay mismatch".into());
    }
    let mut report = first;
    report.replay_exact = true;
    report.hypotheses.hb2_0_protocol_valid =
        report.hypotheses.hb2_0_protocol_valid && report.replay_exact;
    Ok(report)
}
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if !report.hypotheses.hb2_0_protocol_valid {
        return Err("FL-B2 execution gate failed; retain output for diagnosis".into());
    }
    Ok(())
}
...
mod tests {
    ...
    fn campaign_shape_and_replay_are_valid_without_asserting_scientific_outcomes() {
        ...
        assert!(report.hypotheses.hb2_0_protocol_valid);
    }
}
