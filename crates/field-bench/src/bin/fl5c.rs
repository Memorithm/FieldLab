...
struct HypothesisReport {
    h5c_0_null: Check,
    h5c_1_safe_escape: Check,
    h5c_2_structured_noise_control: Check,
    h5c_3_holdout_clean_cue: Check,
    h5c_4_reproducibility: Check,
}
...
struct ValidityReport {
    ...
    both_partitions_wrong_basin_nonempty: Check,
    noiselab_commit_matches: Check,
    no_extra_steps_for_noise: Check,
    fl5_fl5b_case_ids_not_reused: Check,
...
fn run_experiment() -> Result<ExperimentArtifact, Box<dyn Error>> {
    ...
    if let Some((_, selected_spec, selected_cal)) = best.as_ref() {
        ...
        selected_hold.clean_degradation_vs_d0 =
            (hold_d0.clean_recall_fraction - selected_hold.clean_recall_fraction).max(0.0);
        ...
        perm_hold.clean_degradation_vs_d0 =
            (hold_d0.clean_recall_fraction - perm_hold.clean_recall_fraction).max(0.0);

        escape = selected_hold.target_recovery_fraction
            > hold_d0.target_recovery_fraction + f64::EPSILON;
    ...
    let case_ids_ok = cases.iter().all(|case| case.case_id.starts_with("fl5c|"))
        && cases
            .iter()
            .all(|case| !case.case_id.starts_with("fl5|") && !case.case_id.starts_with("fl5b|"));
    let both_clean = cal_clean.len() >= 2 && hold_clean.len() >= 2;
    let both_wrong = !cal_wrong.is_empty() && !hold_wrong.is_empty();
    ...
    let validity = ValidityReport {
        ...
        both_partitions_wrong_basin_nonempty: Check::from_bool(both_wrong),
        noiselab_commit_matches: Check::Pass,
        no_extra_steps_for_noise: Check::Pass,
        fl5_fl5b_case_ids_not_reused: Check::from_bool(case_ids_ok),
        protocol_valid: Check::from_bool(protocol_valid),
    };

    let hypotheses = HypothesisReport {
        h5c_0_null: Check::from_bool(none_survived || !escape),
        h5c_1_safe_escape: Check::from_bool(!none_survived && escape),
        h5c_2_structured_noise_control: Check::from_bool(!none_survived && structured),
        h5c_3_holdout_clean_cue: Check::from_bool(!none_survived && clean_safe),
        h5c_4_reproducibility: Check::from_bool(replay_ok),
    };
    ...
    Ok(ExperimentArtifact {
        experiment: "FL-5C",
        protocol: "nonempty-holdout-clean-cue-basin-escape-v1",
        ...
        fixture: FixtureInfo {
            ...
            description: "FL-5C namespace; 8-node three-pattern Hebbian bank; wrong-basin cues are exact competitors plus single differing-bit flips toward the declared target; clean cues are exact stored patterns under a frozen partition-balanced identifier list (>=2 per side); hard clean-cue calibration filter",
        },
        cases,
    ...
    for target_index in 0..bank.patterns().len() {
        for competitor_index in 0..bank.patterns().len() {
            ...
            for (variant, cue) in cues.into_iter().enumerate() {
                let case_id = format!("fl5c|wb|t{target_index}|c{competitor_index}|k{variant}");
...
fn parse_clean_target_index(case_id: &str) -> Result<usize, Box<dyn Error>> {
    // Expected: fl5c|clean|t{index}|a{alias}
    let parts: Vec<&str> = case_id.split('|').collect();
    if parts.len() != 4 || parts[0] != "fl5c" || parts[1] != "clean" {
...
fn provenance_fingerprint() -> u64 {
    let manifest = format!(
        "fl5c|nodes={NODE_COUNT}|steps={FIELD_STEPS}|dt={FIELD_DT:.17}|mobility={FIELD_MOBILITY:.17}|tilt={CUE_TILT_RADIANS:.17}|amps={AMPLITUDES:?}|ou={OU_THETAS:?}|seeds={SEEDS:?}|perm={PERM_SEED}|noiselab={NOISELAB_COMMIT}|patterns=3|clean={CLEAN_CUE_IDS:?}|fixture=fl5c-wb-competitor+diffbit|frozen-clean-ids-balanced|hard-clean-cue-filter"
    );
...
mod tests {
    ...
    #[test]
    fn fl5c_case_ids_use_dedicated_namespace() {
        let record = partition_record(
            "fl5c|clean|t0|a3".into(),
            "clean_cue",
            0,
            None,
            vec![1; 8],
        ...
        .unwrap();
        assert!(record.case_id.starts_with("fl5c|"));
        assert!(!record.case_id.starts_with("fl5b|"));
    }

    #[test]
    fn frozen_clean_cue_ids_balance_partitions() {
        let mut cal = 0usize;
        let mut hold = 0usize;
        for &case_id in &super::CLEAN_CUE_IDS {
            let record =
                partition_record(case_id.into(), "clean_cue", 0, None, vec![1; 8], false).unwrap();
            match record.partition {
                Partition::Calibration => cal += 1,
                Partition::Holdout => hold += 1,
            }
        }
        assert!(cal >= 2, "calibration clean panel too small: {cal}");
        assert!(hold >= 2, "holdout clean panel too small: {hold}");
        assert_eq!(cal + hold, super::CLEAN_CUE_IDS.len());
    }
